    ...
    if value_inputs:
        ...
        if ng_tool.edit_node_links:
            for socket, input in zip(range(len(value_inputs)), value_inputs):
                ...
                if ng_tool.use_box_groups and ng_tool.use_boxes:
...
class NodeGroupSettings(bpy.types.PropertyGroup):
    ...
    #Node panel settings
    show_node_box_info : BoolProperty(
        name = "Show box info",
        description = "Show box info",
        default = False,
    )
    ...
    show_non_input_groups : BoolProperty(
        name = "show groups with no free inputs",
        description = "show groups with no free inputs",
        default = False,
    )
...
class NG_PT_node_panel(Panel):
    ...
    def draw(self, context):
        ...
        col.label(text = "Make sure you are inside ")
        ...
        if ng_tool.show_node_box_info:
            info_box = col.box()
            info_box.label(text = "To add a group:")
            info_box = info_box.column(align = False)
            info_box.scale_y = 0.8
            info_box.label(text = "1. In the 'interface' panel below ")
            info_box.label(text = "    select the node input that you want to add to a group")
            info_box.label(text = "2. Press the 'New box group' button and enter a name for the group")
            info_box.label(text = "3. If you just want the name to be the same as the selected input")
            info_box.label(text = "    don't type anything in the box")
                            ...
                            if len(node_groups) is not 0:
                                ...
                                if ng_tool.show_group_settings:
                                    ...
                                    if ng_tool.use_boxes:
                                        ...
                                        row.prop(ng_tool, "show_box_info",text = "", emboss = False, icon = "QUESTION")
                                        if ng_tool.show_box_info:
                                            info_box = boxcol.box()
                                            info_box.label(text = "To add a group:")
                                            info_box = info_box.column(align = False)
                                            info_box.scale_y = 0.8
                                            info_box.label(text = "1. Go to the node editor and edit the node group")
                                            info_box.label(text = "2. Find the NGviewer panel in the Node category in the sidebar")
                                            info_box.label(text = "3. Select the node input that you want to add to a group")
                                            info_box.label(text = "4. Press the 'New box group' button and enter a name for the group")
                                            info_box.label(text = "5. If you just want the name to be the same as the selected input")
                                            info_box.label(text = "   don't type anything in the box")
                                    else:
